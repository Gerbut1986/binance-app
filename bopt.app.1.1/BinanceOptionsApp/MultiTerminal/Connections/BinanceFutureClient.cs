namespace MultiTerminal.Connections
{
    ...
    {
        private static List<long> OrderTickets = new List<long>();
        public static List<BinanceNewOrderF> PlacedOrdersUsd_M { get; set; } = new List<BinanceNewOrderF>();
     ...
     //   object lockObject = new object(); // створення об'єкта блокування
                                                ...
                                                try
                                                {
                                                    ...
                                                    zz.BuildZigZag(tasUpdate1.data.Ask, tasUpdate1.data.Bid,
                                                                       tasUpdate1.data.EventTime, (ulong)tasUpdate1.data.Id, TasF, TradeZigZag.GAPbuy_q, TradeZigZag.GAPsell_q,
                                                                        TradeZigZag.GAPbuy_r, TradeZigZag.GAPsell_r);
        ...
        static void WriteMessageToDesktopFile2(string message, string fileName)
        {
            try
            {
       ...
       // object lockObject = new object(); // створення об'єкта блокування
        ...
        public void SubscribePartialDepthStream(string symbol = "BTCUSDT")
        {
            ...
            webSocket.MessageReceived += (sender, e) =>
            {
                ...
                //--------------
               decimal AskV = 0, BidV=0; decimal SumA = 0, SumB=0;
                ...
                lock (lockObject)
                {
                    ...
                    if (SumA > 8 * SumB && AskPrice != PriceRemAsk)
                    {
                        ...
                        if (dominSell >= 3)
                        {
                            ...
                            dominatorSell++; dominatorBuy = 0;MaxImpuleUP = 0; MinImpuleUP = 0; 
                            if (AskPrice > MaxImpuleDN)
                            {
                                MaxImpuleDN = AskPrice;
                            }
                            if (AskPrice < MinImpuleDN || MinImpuleDN == 0)
                            {
                                MinImpuleDN = AskPrice;
                            }
                            DateTimeOffset currentTime = DateTimeOffset.Now;
                            string formattedTime = currentTime.ToString("HH:mm:ss.fff");
                            string symb = TradeZigZag.Symbol; var Len = MaxImpuleDN - MinImpuleDN;

                            if (Len >=50 && dominatorSell >=1)
                            {
                                TradeZigZag.Stakan = 1;
                            ...
                            //  logger.LogInfo($"dominSell, Price={PriceRemAsk}");
                            string FileName = "StakanFuture" + symb + ".txt";
                    ...
                    if (SumB > 8 * SumA && BidPrice != PriceRemBid)
                    {
                        ...
                        if (dominBuy >= 3)
                        {
                            dominatorBuy++; dominatorSell = 0; MinImpuleDN = 0; MaxImpuleDN = 0;
                            if (BidPrice > MaxImpuleUP) 
                            {
                                MaxImpuleUP = BidPrice;
                            }
                            if (BidPrice < MinImpuleUP || MinImpuleUP == 0) 
                            {
                                MinImpuleUP = BidPrice;
                            }
                            // logger.LogInfo($"dominBuy, Price={PriceRemBid}");
                            DateTimeOffset currentTime = DateTimeOffset.Now;
                            string formattedTime = currentTime.ToString("HH:mm:ss.fff");
                            string symb = TradeZigZag.Symbol;
                            decimal Len = 0;
      ...
      //  public static Dictionary<decimal, Plot> TradeLevelPlot = new Dictionary<decimal, Plot>();
        ...
        decimal PreAskPrice=0, PreBidPrice=0, PreAskVolume=0, PreBidVolume = 0, PremarketBuy=0, PremarketSell=0;
                ...
                if (tasUpdateR != null && tasUpdateR != PreUpdate)
                {
                       ...
                       lock (lockObject)
                    {
                       ...
                       //++++++++++++++++++++++++++++++++++++++++++
                        ...
                        if (marketBuy >= 1)
                        {
                            string FileName3 = "Lenta.txt";
                            DateTimeOffset currentTime = DateTimeOffset.Now;
                            string formattedTime = currentTime.ToString("HH:mm:ss.fff");
                            string Mes = $"Time: {formattedTime} | EatingAskResistence = {resistenceAsk} | MarketBuy = {marketBuy} | Price={p} ";
                            if (resistenceAsk == p)
                                WriteMessageToDesktopFile(Mes, FileName3);

                            string Mes2 = $"Price={p} MarkeBuy = {marketBuy} | Ask={AskPrice} Volume={AskVolume} | Bid={BidPrice} Volume={BidVolume}";
                            if(PreMes2 != Mes2) WriteMessageToDesktopFile($"Time: {formattedTime} |  " + Mes2, FileName3);

                            if (AskVolume < BidVolume) 
                            {
                                if (PreAskVolume > 2 * PreBidVolume && PreAskVolume > 10 && PremarketBuy > 2)
                                {
                                    string FileNameA = "Ataca.txt";
                                    string MesA = $"Time: {formattedTime} | ataca 2 | MarkeBuy = {marketBuy} | Ask={AskPrice} Volume={AskVolume} | Bid={BidPrice} Volume={BidVolume} ";
                                    
                                    WriteMessageToDesktopFile(MesA, FileNameA);
                                }
                            }
                        ...
                        if (marketSell >= 1)
                        {
                            ...
                            if (supportBid == p)
                                WriteMessageToDesktopFile($"Time: {formattedTime} | EatingBidSupport = {supportBid} | MarketSell = {marketSell} | Price={p} ", FileName3);
                            string Mes = $"Price={p} MarkeSell = {marketSell}| Ask={AskPrice} Volume={AskVolume} | Bid={BidPrice} Volume={BidVolume}";
                            if(Mes != PreMes)WriteMessageToDesktopFile($"Time: { formattedTime} | " + Mes, FileName3);

                            if (AskVolume > BidVolume)
                            {
                                if (PreAskVolume *2 <  PreBidVolume && PreBidVolume > 10 && PremarketSell > 2)
                                {
                                    string FileNameA = "Ataca.txt";
                                    string MesA = $"Time: {formattedTime} | ataca 1 | MarkeSell = {marketSell} | Ask={AskPrice} Volume={AskVolume} | Bid={BidPrice} Volume={BidVolume} ";
        ...
        public class Echo : WebSocketBehavior
        {
            ...
            {
                BinanceFutureClient.bt = JsonConvert.DeserializeObject<BookTickerF>(e.Data);
               // Send(e.Data);
            }
        }
