    ...
    internal class ConnectorsFactory
    {
        ...
        public IConnector CreateBinanceFuture(IConnectorLogger logger, ManualResetEvent cancelToken, BinanceFutureConnectionModel model, bool is1stLeg = false)
        {
            lock (connectors)
            {
                ...
                if (is1stLeg) 
                    (logger as BinanceOptionsApp.TradeZigZag).model.Leg1.Bfc = (logger as BinanceOptionsApp.TradeZigZag).bfc;
                else (logger as BinanceOptionsApp.TradeZigZag).model.Leg2.Bfc = (logger as BinanceOptionsApp.TradeZigZag).bfc;
                return CreateCRef(client, model.Name);
            }
        }
