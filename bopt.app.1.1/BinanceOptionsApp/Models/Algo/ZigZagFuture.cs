namespace Models.Algo
{
    ...
    public class ZigZagFuture
    {
        ...
        string WW = "0";
        public void BuildZigZag(decimal ask, decimal bid, ulong Time, ulong ID_quark, List<AggTradeFuture> tasLst, decimal avGPbuy, decimal avGPsell, decimal GPbuy, decimal GPsell)
        {
            ...
            if (WaveUP == true && TimeMaxASK < Time) //for DN cur leg
            {
                ...
                if (ID_quark > Pre_id)
                {
                    if (ID_Q_MinBid > LastID_Q_MinBid)
                    {
                       ...
                       // CL = GazerCurrLeg("dn");
                        string CrLg = GazerCurrLeg2("dn", ask , bid);
                        string zzz = CheckZZ();
                        ...
                        string FileName2 = "ScalpFuture" + symb;
                        //---------------------------------------- && ZZF[ZZF.Count - 2].Gap <= TradeZigZag.GAPbuy_q
                        if (CrLg != WW && CrLg != "0")//ZZF.Count >
                        {
                            new _().WriteToTxtFile($"Time: {formattedTime} | OrderGap = {CrLg} | CL: {CrLg} | Ask: {ask}, " +
                            $"Bid: {bid}", FileName2);
                            //TradeZigZag.Order = CrLg;
                            WW = CrLg;//ZZF.Count;
                        }
                        //----------------------------------
                        if (CL != "0")
                        {
                           // DateTimeOffset dateTimeOffset = DateTimeOffset.FromUnixTimeMilliseconds((long)Time);
                             formattedTime = dateTimeOffset.ToString("HH:mm:ss.fff");
                            //string symb = TradeZigZag.Symbol; //
                            //string FileName = "SpikeFuture" + symb;
                            // ----------------------------------------------ZZF[ZZF.Count - 2].Gap >= TradeZigZag.GAPsell_q
                    ...
                    else
                    {
                        try
                        {
                            ...
                            if (Kin == 2 && Triger) //
                            {
                                TradeZigZag.Order = Kin.ToString();
                                long T = (long)Time;
            ...
            if (WaveDN == true && TimeMinBID < Time)//for UP cur leg
            {
                ...
                if (ID_quark > Pre_id)
                {
                    if (ID_Q_MaxAsk > LastID_Q_MaxAsk)
                    {
                       ...
                       // CL = GazerCurrLeg("up");//
                        string CrLg = GazerCurrLeg2("up", ask, bid);
                        string zzz = CheckZZ();
                        //----------------------------
                        DateTimeOffset dateTimeOffset = DateTimeOffset.FromUnixTimeMilliseconds((long)Time);
                        string formattedTime = dateTimeOffset.ToString("HH:mm:ss.fff");
      ...
      //  Zig_Zag Z_Z = new Zig_Zag();
        ...
        #region [Scalp]:
        string Scalp(string tip, decimal MaxCurrBuy, decimal PriceMaxCurrBuy, decimal MaxCurrSell, decimal PriceMaxCurrSell, decimal Ask, decimal Bid )
        {
            string res = "0";
            ...
            if (zzL != null)
            {
                if (zzL.Tip == "UP" && tip == "dn")
                {
                    ...
                    if (zzL.MaxBuyVol > zzL.AvgBuy && zzL.PriceMaxBuy >= PriceMaxLevelBuy) 
                    { 
                        if(MaxCurrBuy >= zzL.AvgBuy && PriceMaxCurrBuy <= zzL.PriceMaxBuy && Ask <= PriceMaxCurrBuy)
                        {
                            res = "2";
                        }

                        if (MaxCurrBuy < zzL.AvgBuy && PriceMaxCurrBuy <= zzL.PriceMaxBuy && Ask <= PriceMaxCurrBuy && Ask < PriceMaxCurrSell)
                        {
                            res = "2";
                        }

                        if(Ask <= zzL.PriceMaxBuy)
                        {
                            res = "200";
                        }
                       
                    }
                    if (MaxCurrBuy < MaxCurrSell && MaxCurrSell > zzL.AvgSell && zzL.AvgBuy < zzL.AvgSell) { res = res + 1000; }

                    if (zzL.PriceMaxSell <= PriceMaxLevelSell)
                    {
                        if (MaxCurrBuy < MaxCurrSell && MaxCurrSell > zzL.AvgSell && Bid >= PriceMaxCurrSell) { res = res + 1001; }
                    }
                ...
                {
                    // 1 ----- strong ZigZag, norm CurrLeg 
                    if (zzL.MaxSellVol > zzL.AvgSell && zzL.PriceMaxSell <= PriceMaxLevelSell)
                    {
                        if (MaxCurrSell >= zzL.AvgSell && PriceMaxCurrSell >= zzL.PriceMaxSell && Bid >= PriceMaxCurrSell)
                        {
                            res = "1";
                        }

                        if (MaxCurrSell < zzL.AvgSell && PriceMaxCurrSell >= zzL.PriceMaxSell && Bid >= PriceMaxCurrSell && Bid > PriceMaxCurrBuy)
                        {
                            res = "1";
                        }

                        if (Bid >= zzL.PriceMaxSell)
                        {
                            res = "100";
                        }
                    }

                    if (MaxCurrBuy > MaxCurrSell && MaxCurrBuy > zzL.AvgBuy && zzL.AvgBuy > zzL.AvgSell) { res = res + "2000"; }

                    if (zzL.PriceMaxBuy >= PriceMaxLevelBuy)
                    {
                        if (MaxCurrBuy > MaxCurrSell && MaxCurrBuy > zzL.AvgBuy && Ask <= PriceMaxCurrBuy) { res = res + "2002"; }
                    }
        ...
        private string GazerCurrLeg2(string tip, decimal ask, decimal bid)
        {
            string res = "0";
            if (CurrLegPlot.Any() && CurrLegPlot.Count > 1)
            {
                try
                {
                    ...
                    //----------------------------------------------------------------
                    res = Scalp(tip, MaxCurrBuy, PriceMaxCurrBuy, MaxCurrSell, PriceMaxCurrSell, ask, bid);
                    string Ser = "0";

                    if (tip == "up")
                    {
                        if ((CurrMaxPrice - PriceMaxCurrSell) * 2 < (PriceMaxCurrSell - CurrMinPrice))
                        {
                            Ser = "11";
                        }
                    }
                    if (tip == "dn")
                    {
                        if ((PriceMaxCurrBuy - CurrMinPrice) * 2 < (CurrMaxPrice - PriceMaxCurrBuy))
                        {
                            Ser = "22";
                        }
                    }
