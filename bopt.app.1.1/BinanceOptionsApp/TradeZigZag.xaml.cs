...
namespace BinanceOptionsApp
{
    ...
    public partial class TradeZigZag : UserControl, IConnectorLogger, ITradeTabInterface
    {
        ...
        private bool LimitBuy = false, LimitSell = false, LockSell=false, LockBuy=false;
        private string ticketBuy = "0", ticketSell = "0", ticketBuyLock = "0", ticketSellLock = "0";
       ...
       // private DispatcherTimer timerEvent;
        ...
        {
            new Socket_Wnd(bfc, model.Leg1.Symbol).ShowDialog();
        }
        ...
        int indexSymb=0; decimal actualPosition = 0; decimal LastBuyPrice = 0, LastSellPrice = 0, EntryPrice=0, TakeProfit=0;
        decimal LockBuyPrice = 0, LockSellPrice = 0;
        int ClientOrderId = 0;
            ...
            while (!threadStop.WaitOne(model.SleepMs))
            {
                ...
                if (model.Leg2.Bid != 0 && model.Leg1.Ask != 0)
                {
                    ...
                    if (model.Leg2.Bid != 0 && model.Leg1.Ask != 0 && model.Leg2.Ask != 0 && model.Leg1.Bid != 0)
                    {
                        ...
                        if ((DateTime.Now - StrtInterval).TotalSeconds > 5)
                        {
                        ...
                        if (GapBuy != 0 && avgGapBuy != 0)
                        {
                            ...
                            GAPbuy_q = avgGapBuy;//GapBuy
                            GAPbuy_r = GapBuy;//GapBuy
                ...
                //if ((DateTimeOffset.UtcNow.ToUnixTimeSeconds() - LastTime) >= 8)
                //{
                //    LastTime = DateTimeOffset.UtcNow.ToUnixTimeSeconds();
                //}
                decimal wantPriceBuy = 0, wantPriceSell = 0;

                if ((DateTimeOffset.UtcNow.ToUnixTimeSeconds() - LastTime2) >= 5)
                {
                    if (bfc != null)
                    {
                                ...
                                for (int eu = 0; eu < positions.Count; eu++)
                                {
                                    ...
                                    if (ss == model.Leg1.Symbol.ToUpper())
                                    {
                                        ...
                                        if (decimal.TryParse(positions[eu].EntryPrice, out EntryPrice))
                                        {
                                            ...
                                            {
                                                double TP = (double)Leg * 0.5;
                                                if (actualPosition > 0) TakeProfit = EntryPrice + (decimal)Math.Round(TP, 2);
                                                else if (actualPosition < 0) TakeProfit = EntryPrice - (decimal)Math.Round(TP, 2);
                                            }
                                        }
                    ...
                    {
                        LimitBuy = false;
                        wantPriceSell = HFT("up", Ask1, Bid1, "Cls");
                    }

                    if (actualPosition < 0)
                    {
                        LimitSell = false;
                        wantPriceBuy = HFT("dn", Ask1, Bid1, "Cls");
                    }

                    if (actualPosition == 0)
                    {
                        if (!LimitBuy) LockSell = false;
                        if (!LimitSell) LockBuy = false;
                        calcOpenOrderSell = 0;
                        calcOpenOrderBuy = 0;
                    }
                ...
                //******************************************************************
               // if (actualPosition > 0 && TakeProfit !=0 && Bid1 >= TakeProfit) CloseAllBuy = true;
              //  else if (actualPosition < 0 && TakeProfit !=0 && Ask1 <= TakeProfit) CloseAllSell = true; 
               ...
               // int ORD = 0;
                ...
                //if (ORD == 3) CloseAllBuy = true;
                //if (ORD == 4) CloseAllSell = true;
                //if (Order.Contains("1") && (Bid1 > LastSellPrice + minStep || LastSellPrice == 0)) OpenShort = true;
                //if (Order.Contains("2") && (Ask1 < LastBuyPrice - minStep || LastBuyPrice == 0)) OpenLong = true;
                //  if(model.Leg1.Bid>0) OpenLong = true;
                // 
                //^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
                ...
                if (Stakan == 2)
                {
                    wantPriceBuy = HFT("up", Ask1, Bid1, "Opn");
                    Stakan = 0;
                    OpenLong = true;
                }
                if (Stakan == 1)
                {
                    wantPriceSell = HFT("dn", Ask1, Bid1, "Opn");
                    Stakan = 0;
                    OpenShort = true;
                }
                long id2 = 0; 

                if (LimitBuy || LimitSell || LockBuy || LockSell)
                {
                    var of = BinanceFutureClient.PlacedOrdersUsd_M;
                    int indx = of.Count(); 
                ...
                if (Pos)
                {
                    if (!LockBuy && actualPosition < 0) 
                    {
                        if (model.AllowOpen)
                        {
                            model.LogInfo($"Try place BuyLimit, Bid: {model.Leg1.Bid} Ask:  {model.Leg1.Ask}");
                            ...
                            decimal prc = wantPriceBuy + 5;
                            ticketBuyLock = OpenPos(model.Leg1.Symbol, prc, lot, "1Leg", FillPolicy.GTC, OrderSide.Buy, deviationBuy, OrderType.Limit);
                            if (ticketBuyLock != "0")
                            {
                                ClientOrderId++;
                                LockBuyPrice = prc;
                                model.LogInfo($"Placed lock BuyLimit ");
                                LockBuy = true;
                    ...
                    if(!LockSell && actualPosition > 0)
                    {
                        if (model.AllowOpen)
                        {
                            model.LogInfo($"Try place SellLimit, Bid: {model.Leg1.Bid} Ask:  {model.Leg1.Ask}");

                            long milliseconds = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds();
                            DateTimeOffset dateTimeOffset = DateTimeOffset.FromUnixTimeMilliseconds(milliseconds);
                            string formattedTime = dateTimeOffset.ToString("HH:mm:ss.fff");

                            decimal prc = wantPriceSell - 5;
                            ticketSellLock = OpenPos(model.Leg1.Symbol, prc, lot, "1Leg", FillPolicy.GTC, OrderSide.Sell, deviationSell, OrderType.Limit);
                            if (ticketSellLock != "0")
                            {
                                ClientOrderId++;
                                LockSellPrice = prc;
                                model.LogInfo($"Placed lock SellLimit ");
                                LockSell = true;
                    ...
                    //---- modify lock ---
                    if(LockBuy && actualPosition < 0 && wantPriceBuy >0 && wantPriceBuy != LockBuyPrice)
                    {
                        if (model.AllowOpen)
                        {
                            ...
                            DateTimeOffset dateTimeOffset = DateTimeOffset.FromUnixTimeMilliseconds(milliseconds);
                            string formattedTime = dateTimeOffset.ToString("HH:mm:ss.fff");
                            decimal prc = wantPriceBuy + 5;///ticketBuyLock, ticketBuyLock
                            ...
                            id2 = of[indx - 1].OrderId;
                            ID = id2.ToString();
                            if (OrderModify(model.Leg1.Symbol, "1Leg", ClientOrderId.ToString(), ID, OrderSide.Buy, prc, lot))
                            {
                                LockBuyPrice = prc;
                                model.LogInfo($"Modify BuyLimit (1Leg)");
                    ...
                    if(LockSell && actualPosition > 0 && wantPriceSell>0 && wantPriceSell != LockSellPrice)
                    {
                        if (model.AllowOpen)
                        {
                            ...
                            id2 = of[indx - 1].OrderId;
                            ID = id2.ToString();
                            if (OrderModify(model.Leg1.Symbol, "1Leg", ClientOrderId.ToString(), ID, OrderSide.Sell, prc, lot))//(model.Leg1.Symbol, prc, lot, "1Leg",
                            {
                                LockSellPrice = prc;
                ...
                {
                    if (!LimitBuy && OpenLong && calcOpenOrderBuy < grid)
                    {
                        if (model.AllowOpen)
                        {
                            ...
                            decimal prc = wantPriceBuy +5; //  model.Leg1.Bid;
                            ticketBuy = OpenPos(model.Leg1.Symbol, prc, lot, "1Leg", FillPolicy.GTC, OrderSide.Buy, deviationBuy, OrderType.Limit); 
                            if (ticketBuy!="0")
                            {
                                ClientOrderId++;
                                LastBuyPrice = prc;
                    ...
                    if (!LimitSell && OpenShort && calcOpenOrderSell < grid)
                    {
                        if (model.AllowOpen)
                        {
                            ...
                            decimal prc = wantPriceSell - 5;//model.Leg1.Ask+500;  
                            ticketSell = OpenPos(model.Leg1.Symbol, prc, lot, "1Leg", FillPolicy.GTC, OrderSide.Sell, deviationSell, OrderType.Limit);
                            if (ticketSell != "0")
                            {
                                ClientOrderId++;
                                LastSellPrice = prc;
                    ...
                    //---------- modify ---------
                    if (LimitSell && OpenShort && LastSellPrice != wantPriceSell)
                    {
                        if (model.AllowOpen)
                        {
                            model.LogInfo($"Try Modify Sell order, ");
                            model.LogInfo($"Support: {priceSupport} VolS:{volSupport} Resistence: {priceResistence} VolR:{volResistence}");
                            ...
                            {
                                LastSellPrice = prc;
                                model.LogInfo($"Modify SellLimit (1Leg)");
                                OpenShort = false; //Pos = true; PosSell = true; 
                    ...
                    if (LimitBuy && OpenLong && LastBuyPrice != wantPriceBuy)
                    {
                        if (model.AllowOpen)
                        {
                            model.LogInfo($"Try Modify Buy order, ");
                            model.LogInfo($"Support: {priceSupport} VolS:{volSupport} Resistence: {priceResistence} VolR:{volResistence}");

                            long milliseconds = (long)(DateTimeOffset.UtcNow.ToUnixTimeMilliseconds());
                            DateTimeOffset dateTimeOffset = DateTimeOffset.FromUnixTimeMilliseconds(milliseconds);
                            string formattedTime = dateTimeOffset.ToString("HH:mm:ss.fff");
                            decimal prc = wantPriceBuy + 5;//model.Leg1.Ask+500;  

                            if (OrderModify(model.Leg1.Symbol, "1Leg", ClientOrderId.ToString(), ID, OrderSide.Buy, prc, lot))//(model.Leg1.Symbol, prc, lot, "1Leg",//FillPolicy.GTC, OrderSide.Sell, deviationSell, OrderType.Limit))
                            {
                                LastBuyPrice = prc;
                                model.LogInfo($"Modify BuyLimit (1Leg)");
                    ...
                    //---------- delete ----------
                    if (LimitSell && OpenLong)
                    {
                        if (model.AllowOpen)
                        {
                            model.LogInfo($"Try Delete Sell order, ");
                            model.LogInfo($"Support: {priceSupport} VolS:{volSupport} Resistence: {priceResistence} VolR:{volResistence}");
                            ...
                            if (OrderDelete(model.Leg1.Symbol, OrderSide.Sell, OrderType.Limit,"0",ID,ID))//(model.Leg1.Symbol, prc, lot, "1Leg", //FillPolicy.GTC, OrderSide.Sell, deviationSell, OrderType.Limit))
                            {
                                LimitSell = false;
                                model.LogInfo($"Delete SellLimit (1Leg)");
                                WriteMessageToDesktopFile($"Time: { formattedTime} | Umova = 2, Delete Sell || Ask: { model.Leg1.Ask}, " +
                    ...
                    if (LimitBuy && OpenShort)
                    {
                        if (model.AllowOpen)
                        {
                            model.LogInfo($"Try Delete Buy order, ");
                            model.LogInfo($"Support: {priceSupport} VolS:{volSupport} Resistence: {priceResistence} VolR:{volResistence}");
                            ...
                            if (OrderDelete(model.Leg1.Symbol, OrderSide.Buy, OrderType.Limit, "0", ID, ID))//(model.Leg1.Symbol, prc, lot, "1Leg", //FillPolicy.GTC, OrderSide.Sell, deviationSell, OrderType.Limit))
                            {
                                LimitBuy = false;
                                model.LogInfo($"Delete BuyLimit (1Leg)");
        ...
        private decimal HFT(string tip, decimal Ask1, decimal Bid1, string Action)
        {
            decimal res = 0; decimal StartPrice = 0, FinishPrice = 0; decimal parm = 0;
            if (Action == "Opn" && tip=="dn") parm = 50;
            if (Action == "Cls") parm = 100;
            StartPrice = Ask1; FinishPrice = Ask1 + 100;
            var StakanA = BinanceFutureClient.marketDepthList
            ...
            StartPrice = Bid1; FinishPrice = Bid1 - 100; 
            var StakanB = BinanceFutureClient.marketDepthList
                              .Where(pair => pair.Key <= StartPrice && pair.Key >= FinishPrice)
                              .OrderBy(pair => pair.Key) // Сортуємо за зростанням значення ключа
                              .ToDictionary(pair => pair.Key, pair => pair.Value.DeepCopy());
            ...
            if (tip == "up")
            {
               // TradeZigZag.Stakan = 2;
                string FileName4 = "HFTFuture" + symb + ".txt";
                if(Stakan == 2)
        ...
        public string OpenPos(string symb, decimal price, decimal LOT, string type, FillPolicy policy, OrderSide bs, decimal gap, OrderType orderType)
        {
            string ticket = "0";
